...
class sqliteDb {
  TB = {
    ...
    TMP: { FIELD: { date: "date" } },
    STATS: {
      FIELD: Object.keys(D.STATS_KEY_MAP).reduce((p, c) => {
        p[c] = c;
        return p;
      }, {}),
    },
  ...
  getTblName(name) {
    if (["ACCOUNT", "SEASON", "YAKU", "TMP", "MEMBERS"].indexOf(name) > -1) return this.TB.NAME[name];
    ...
    return new Promise((resolve, reject) => {
      db.serialize(() => {
        let sqlList = [
          ...
          `create table if not exists ${this.getTblName("STATS")}(${Object.values(this.TB.STATS.FIELD)});`,
          ...
          `create table if not exists ${this.getTblName("MEMBERS")}(${Object.values(this.TB.MEMBERS.FIELD)});`,
        ];
        try {
...
//       `create table if not exists ${dbc.TB.NAME.STATS}_${year}(${Object.values(dbc.TB.STATS.FIELD)});`,
